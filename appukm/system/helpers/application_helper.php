...
if ( ! function_exists('pages2'))
{
	function pages2($halaman2, $jmlhalaman, $url, $id=""){
		...
		if ($halaman2 > 1){
			$previous = $halaman2-1;
			echo "<a href='".site_url().$url."/1".$id."'  class='page-num'><<</a>
			<a href='".site_url().$url."/".$previous.$id."'  class='page-num'><</a> ";
		} else {
			echo "<span class='current page-num'><<</span>
				  <span class='current page-num'><</span> ";
		}
		if ($halaman2 < $jmlhalaman){
			$next = $halaman2+1;
			echo "<a href='".site_url().$url."/".$next.$id."' class='page-num'>></a>
			      <a href='".site_url().$url."/".$jmlhalaman.$id."'  class='page-num'>>></a> ";
		} else {
			echo "<span class='current page-num'>></span>
				  <span class='current page-num'>>></span> ";
		}
